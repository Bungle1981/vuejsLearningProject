<template>
    <form @submit.prevent="submitForm">
        ...
        <div class="form-control" :class="{invalid: !lastName.isValid}">
            ...
            <p class="invalid" v-if="!firstName.isValid" >Last name must not be empty.</p>
        </div>
        <div class="form-control" :class="{invalid: !description.isValid}">
            <label for="description">Description</label>
            <textarea id="description" rows="5" v-model.trim="description.val" @blur="clearValidity('description')"></textarea>
            <p class="invalid" v-if="!firstName.isValid" >Description must not be empty.</p>
        </div>
        <div class="form-control" :class="{invalid: !rate.isValid}">
            <label for="hourlyrate">Hourly rate</label>
            <input type="number" id="hourlyrate" v-model.trim.number="rate.val" @blur="clearValidity('rate')"/>
            <p class="invalid" v-if="!firstName.isValid" >Rate must not be empty.</p>
        </div>
        <div class="form-control" :class="{invalid: !areas.isValid}">
            ...
            <p class="invalid" v-if="!firstName.isValid" >You must select an area of expertise.</p>
        </div>
        <p v-if="!formIsValid">Please check the above errors and submit again.</p>
